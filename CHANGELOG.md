...
- New CI validation: `scripts/validate_cross_references.py` — semantic cross-reference integrity checks for work artifacts, signal supersession chains, governance exception references, integration registry category alignment (`CONFIG.yaml` ↔ `org/integrations/categories/`), and quality-policy file references including bare `.md` mentions. Runs as blocking `validate-cross-references` job in `validate.yml`. Closes #114.
